...
[![Version](https://img.shields.io/badge/version-1.1.1-blue?style=flat-square)](https://github.com/myousefg/Grabbr/releases/latest)
...
1. Download **`Grabbr-Setup-1.1.1.exe`** from the
...
- **Verify the download (optional):** each release also ships `checksum.txt`; compare it with
  `Get-FileHash Grabbr-Setup-1.1.1.exe` in PowerShell.
...
3. electron-builder ->  dist\Grabbr-Setup-1.1.1.exe + checksum.txt
```
